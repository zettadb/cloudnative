...
-- CUSTOMER TABLE
   ...
   query = string.format([[
    ...
    partition by range(c_id) %s %s]],
    --create table customer%d_5 partition of customer%d for values from (1) to (500);
    --create table customer%d_10 partition of customer%d for values from (501) to (1000);
    --create table customer%d_15 partition of customer%d for values from (1001) to (1500);
    --create table customer%d_20 partition of customer%d for values from (1501) to (2000);
    --create table customer%d_25 partition of customer%d for values from (2001) to (2500);
    --create table customer%d_30 partition of customer%d for values from (2501) to (3000) %s %s]],
      table_num, engine_def, extra_table_options)
...
-- HISTORY TABLE
   ...
   for his=1,3 do
	   ...
	   ]], table_num, his, table_num, low, high, engine_def, extra_table_options )
	   con:query(query)
   ...
   for ord=1,3 do
           ...
           ]], table_num, ord, table_num, low, high, engine_def, extra_table_options )
           con:query(query)
...
-- NEW_ORDER table
   ...
   query = string.format([[
	...
	)partition by range(ol_o_id) %s %s]],
      table_num, engine_def, extra_table_options)
   ...
   for oli=1,6 do
           ...
           ]], table_num, oli, table_num, low, high, engine_def, extra_table_options )
           con:query(query)
...
-- STOCK table
   ...
   for sto=1,5 do
           ...
           ]], table_num, sto, table_num, low, high, engine_def, extra_table_options )
           con:query(query)
   ...
   for ite=1,5 do
           ...
           ]], table_num, ite, table_num, low, high, engine_def, extra_table_options )
           con:query(query)
   ...
   con:bulk_insert_done()
    ...
    con:query("CREATE INDEX idx_customer"..i.." ON customer"..i.." (c_w_id,c_d_id,c_last,c_first);")--CREATE INDEX idx_orders1 ON orders1 (o_w_id,o_d_id,o_c_id,o_id;)
