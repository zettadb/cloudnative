...
def stop_with_config(jscfg, comf, machines, args):
    ...
    for node in nodemgr['nodes']:
        ...
        addToCommandsList(commandslist, node['ip'], ".", 'bash ./stop_instances.sh %d %s %s >& stop.log || true' % (
            node['brpc_http_port'], mach['basedir'], args.product_version))
    ...
    # stop the meta nodes, no need any more, since stop_instances.sh has stopped it.
    # for node in meta['nodes']:
    #    if args.autostart:
    #        servname = 'kunlun-storage-%d.service' % node['port']
    #        generate_systemctl_stop(servname, node['ip'], commandslist)
...
def start_with_config(jscfg, comf, machines, args):
    ...
    for node in nodemgr['nodes']:
        if args.autostart:
            ...
            addToCommandsList(commandslist, node['ip'], ".", 'bash ./start_instances.sh %d %s %s >& start.log || true' % (
                node['brpc_http_port'], mach['basedir'], args.product_version))
            addToCommandsList(commandslist, node['ip'], '.', "bash start-nodemgr-%d.sh </dev/null >& run.log &" % node['brpc_http_port'])
