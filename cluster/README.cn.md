...
2 所有集群节点所在机器已经正确设置好用户，节点将以该用户启动，该用户能够运行sudo而不需要密码, 且该用户属于或具有一个同名的用户组
...
如果该默认目录没有在命令行中指定，则'/kunlun'将作为默认目录，用户指定该参数的话，则需要使用绝对路径。
...
* 元数据集群为一个存储节点复制组，一主多备，内部含有3个或3个以上的存储节点。
