...
defarg="--defaults-file=$KUNLUNBASE/percona-8.0.18-bin-rel/etc/my_$port.cnf"

# command to get the executed gtids is:
isslave=`mysql $defarg -uroot -proot -D mysql -s -e 'select @@read_only' 2>/dev/null | tail -1`
...
if test "$gtid" =  "0"; then
	...
	echo "$gtid" > gtid
	rm -f backup.tar.gz
	cp -f $KUNLUNBASE/percona-8.0.18-bin-rel/etc/my_$port.cnf my.cnf
	tar -czf backup.tar.gz base gtid my.cnf
	rm -fr base
else
	fullpath=`pwd`
	binlogidx=`mysql $defarg -uroot -proot -D mysql -e 'select @@log_bin_index' 2>/dev/null  | tail +2`
	mkdir -p logfiles
	(
		cd $logdir
		python /$KUNLUNBASE/backup/check_gtid.py $gtid $binlogidx | while read f; do
			echo "Copying $f .... "
			cp -f $f $fullpath/logfiles
		done
	)
	gtid=`mysql $defarg -uroot -proot -D mysql -e 'select @@global.gtid_executed' 2>/dev/null  | tail +2 | sed 's/\\\\n//g'`
	echo "$gtid" > gtid
	rm -f backup.tar.gz
